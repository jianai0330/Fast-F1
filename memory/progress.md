...
- [x] 「最近一站」静态卡片已移除，替换为可点击的「最新赛站数据」入口
  ...
  - 带有按压态反馈（scale + opacity），提升交互感
  - 清理了不再使用的 `formatRaceStatus()` 函数

### 全局交互升级 P0（2026-05-28）
- [x] **统一按压反馈**：6 个页面（index/event/news/forum/standings/glossary）所有可点击元素加 `:active` 按压态
  - 卡片类：scale(0.97) + opacity(0.88)
  - 按钮/Tab/入口类：opacity(0.6~0.8)
  - 统一 `transition: transform 0.12s ease, opacity 0.12s ease`
- [x] **骨架屏替代文字 loading**：index / event / standings 三页
  - 首页：模拟赛历卡片骨架（圆角矩形 + 文字条）
  - 赛事页：排位表模拟 5 行骨架行 + 赛道信息 4 张骨架卡 + 正赛 3 张骨架卡
  - 积分榜：模拟 8 行排行骨架（位置条 + 车手条 + 积分条）
  - 全部使用 `shimmer` 脉冲动画
- [x] **赛历状态标识**：
  - 已完赛站：整体 opacity 半透明 + 灰色文字
  - 下一站：红色渐变角标 + NEXT 悬浮徽章 + 红色提示文字
  - 未来站：保持正常显示
...
- 首页赛道线稿再次微调：固定描边宽度继续从 2.8 下调到 2.2，先用最小改动验证赛道缩略图是否更接近轻薄、清晰的视觉效果。
- 首页赛道承接方式继续校正：列表右侧改为固定尺寸的预览槽，图像本身缩入槽内并水平垂直居中，优先解决“看起来没居中、像在飘”的问题。
